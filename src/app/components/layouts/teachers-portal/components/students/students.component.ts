...
export class StudentsComponent {
  ...
  onItemSelect(item: any) {
    ...
    for (let i = 0; i < this.stuData.length; i++) {
      this.filteredStd = this.stuData.filter(
        (stu: any) => stu[4].toLowerCase().trim() == this.selectedItems[0].item_text.toLowerCase()
      );
    }
  ...
  onSerch(event: any) {
    ...
    if (this.search != '') {
      ...
      this.serchData = serchStu.filter((stu: any) =>
        stu[0].toLowerCase().includes(this.search.toLowerCase())
      );
  ...
  std: string = ""
  showSection(section: string, item?: any) {
    this.currentSection = section;
    this.std = item.item_text
    this.filteredStd = this.stuData.filter(
      (stu: any) =>
        stu[4].toLowerCase().trim() == this.std.toLowerCase()
    );
